# specbridge/eval/candidates_by_adduct.py
...
class AdductFilteredDataset(Dataset):
    """Wrapper dataset that filters MassSpecGymDataset by adduct."""
    def __init__(self, base_dataset: MassSpecGymDataset, adducts: Optional[set[str]] = None):
        super().__init__()
        self.base_dataset = base_dataset
        
        if adducts is not None:
            # Normalize adduct strings (strip whitespace)
            self.adducts = {str(a).strip() for a in adducts}
            # Find matching indices by accessing _records directly
            self._indices = []
            if hasattr(base_dataset, '_records'):
                for i in range(len(base_dataset._records)):
                    rec = base_dataset._records[i]
                    # Get adduct from record (stored in 'adduct' field or params)
                    adduct = str(rec.get("adduct") or rec.get("params", {}).get("ADDUCT") or rec.get("params", {}).get("adduct") or "").strip()
                    if adduct in self.adducts:
                        self._indices.append(i)
            else:
                # Fallback: iterate through dataset (less efficient)
                for i in range(len(base_dataset)):
                    item = base_dataset[i]
                    # Try to get adduct from params in the returned item
                    # This is a fallback if _records is not accessible
                    # Note: this requires accessing the underlying data structure
                    # For now, we'll assume _records is available
                raise ValueError("Cannot filter by adduct: base dataset does not have _records attribute")
        else:
            self.adducts = None
...
@torch.no_grad()
def main():
    ap = argparse.ArgumentParser("Per-spectrum candidate ranking filtered by adduct (PKL: true_smi -> [candidate_smi,...])")
    ...
    ap.add_argument("--adduct", type=str, required=True, help="Comma-separated list of adducts to filter by (e.g. '[M+H]+,[M+Na]+')")
    ...
    adduct_filter = None
    if args.adduct:
        adduct_list = [a.strip() for a in args.adduct.split(",") if a.strip()]
        adduct_filter = set(adduct_list)
        print(f"[filter] filtering by adducts: {sorted(adduct_filter)}")

    # Dataset (fold-aware)
    folds = None
    if args.fold_query:
        folds = {f.strip().lower() for f in args.fold_query.split(",") if f.strip()}
    ds_base = MassSpecGymDataset(args.mgf, args.meta_json, folds=folds)
    
    # Apply adduct filtering
    if adduct_filter:
        ds = AdductFilteredDataset(ds_base, adduct_filter)
        print(f"[filter] filtered dataset: {len(ds_base)} -> {len(ds)} spectra")
    else:
        ds = ds_base

    formula_vocab = max(2, getattr(ds_base, "_formula_vocab", 0) or 32)
    adduct_vocab  = max(2, getattr(ds_base, "_adduct_vocab", 0) or 16)
    charge_vocab  = max(2, getattr(ds_base, "_charge_vocab", 0) or 8)
        ...
        if args.use_true:
            ...
            if args.use_mapped:
                ...
                if args.deterministic_map or (lv is None):
                    ...
                    z_query = model.mapB.sample(mu, lv, deterministic=False)  # [B,D]
            else:
    ...
    print(f"[filter] adducts={sorted(adduct_filter) if adduct_filter else 'all'}")
